...
export default function SwapScreen() {
  ...
  const selectToken = (mint: string) => {
    if (pickerTarget === "input") {
      ...
      if (mint === inputAmount) setInputToken(outputToken);
      setOutputToken(mint);
    }
  ...
  const handleSwap = async () => {
    ...
    if (!wallet.quotedata) {
      return Alert.alert("No Quote", "Enter an Amount to get Quote first");
    }
            ...
            <FlatList
              ...
              renderItem={({ item }) => {
                ...
                return (
                  ...
                  >
                    <View
                      style={[s.tokenIcon, { backgroundColor: info.color }]}
                    >
        ...
        <ScrollView style={s.scroll} contentContainerStyle={s.content}>
          ...
          {isDevnet && (
            <View style={s.devnetWarning}>
              ...
              <Text style={s.devnetText}>
                Jupiter Swap only works on Mainnet. Switch network to Swap
              </Text>
            </View>
          )}
          {/* From Token Card */}
          ...
          {wallet.connected ? (
            <TouchableOpacity
              style={[
                s.swapBtn,
                (!wallet.quotedata || wallet.swaping || isDevnet) &&
                  s.swapBtnDisabled,
              ]}
              onPress={handleSwap}
              disabled={!wallet.quotedata || wallet.swaping || isDevnet}
            >
              {wallet.swaping ? (
